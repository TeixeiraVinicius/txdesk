      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Chamados</li>
          </ol>
      ...
      <div class="row mt-4">
        <div class="col-6">
          <div class="card ">
            ...
            <div class="card-header">
              <h5 class="mb-0">Pesquisar</h5>
              <p class="text-sm mb-0">Filtros de Informações</p>
            </div>
            <div class="card-body">
              <form action="{{route('pesquisar.chamados')}}" method="post">
              @csrf
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <input type="text" class="form-control" name="pesquisar" placeholder="ID, Cliente, Produto, Pedido, Data de Abertura...">
                  <button type="submit" class="btn bg-gradient-info" style="margin-top: 15px;">Pesquisar</button>
              </form>
            </div>  
        ...
        <div class="col-6">
          <div class="card ">
            <!-- Card header -->
            <div class="card-header">
              <h5 class="mb-0">Adicionar Assitência</h5>
              <p class="text-sm mb-0">Cadastrar nova Assitência</p>
            </div>
            <div class="card-body">
              <a href="{{route('add.chamados')}}">
              <button type="button" class="btn - bg-gradient-info">Adicionar</button>
              </a>
            </div>
      ...
      <div class="row mt-4">
        <div class="col-12">
          <div class="card">
            <!-- Card header -->
            <div class="card-header">
              <h5 class="mb-0">Tabela de Assistências</h5>
              ...
              <table class="table table-flush" id="datatable-basic">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th colspan="3" class="text-right text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ações</th>
                  </tr>
                </thead>
                @foreach ($chamado as $chamados)
               <tbody>
                 <tr>
                 <td class="text-sm font-weight-normal">{{$chamados->id}}</td>
                 <td class="text-sm font-weight-normal">{{$chamados->nome}}</td>
                 <td class="text-sm font-weight-normal">{{$chamados->produto}}</td>
                 <td class="text-sm font-weight-normal">{{$chamados->status}}</td>
                 <td class="text-sm font-weight-normal">{{\Carbon\Carbon::parse($chamados->dataabertura)->format('d/m/Y')}}</td>
                 <td class="text-sm font-weight-normal td-actions text-right">
                  <button type="button" rel="tooltip" class="btn btn-info" onclick="window.location.href='/chamados/visualizar-chamado/{{$chamados->id}}'">
                        <i class="fa fa-search"></i>
                      </button> 
                  </td>
                 <td class="text-sm font-weight-normal td-actions text-right">
                  <button type="button" rel="tooltip" class="btn btn-success" onclick="window.location.href='/chamados/editar-chamado/{{$chamados->id}}'">
                        <i class="fa fa-edit"></i>
                      </button> 
                  </td>
                  <td class="text-sm font-weight-normal">    
                      <form method="post" action="{{ route('deletar.chamados', [$chamados->id]) }}">
                      @csrf
