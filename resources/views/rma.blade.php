      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">RMAs</li>
          </ol>
      ...
      <div class="row mt-4">
        <div class="col-6">
          <div class="card ">
            ...
            <div class="card-body">
              <form action="{{route('pesquisar.rmas')}}" method="post">
              @csrf
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                  <input type="text" class="form-control" name="pesquisar" placeholder="ID, Produto, Fabricante...">
        ...
        <div class="col-6">
          <div class="card ">
            <!-- Card header -->
            <div class="card-header">
              <h5 class="mb-0">Adicionar RMA</h5>
              <p class="text-sm mb-0">Cadastrar novo RMA</p>
            </div>
            <div class="card-body">
              <a href="{{route('add.rmas')}}">
              <button type="button" class="btn - bg-gradient-info">Adicionar</button></a>
            </div>
      ...
      <div class="row mt-4">
        <div class="col-12">
          <div class="card">
            <!-- Card header -->
            <div class="card-header">
              <h5 class="mb-0">Tabela de RMAs</h5>
              ...
              <table class="table table-flush" id="datatable-basic">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Fabricante</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Status</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Data de Abertura</th>
                    <th colspan="2" class="text-right text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ações</th>
                  </tr>
                </thead>
               ...
               <tbody>
                 <tr>
                 <td class="text-sm font-weight-normal">{{$rmas->id}}</td>
                 <td class="text-sm font-weight-normal">{{$rmas->produto}}</td>
                 <td class="text-sm font-weight-normal">{{$rmas->fabricante}}</td>
                 <td class="text-sm font-weight-normal">{{$rmas->status}}</td>
                 <td class="text-sm font-weight-normal">{{\Carbon\Carbon::parse($rmas->dataabertura)->format('d/m/Y')}}</td>
                 <td class="td-actions text-right">
                  <button type="button" rel="tooltip" class="btn btn-success" onclick="window.location.href='/rmas/editar-rma/{{$rmas->id}}'">
                        <i class="fa fa-edit"></i>
                      </button> 
                  </td>
                  <td>    
                      <form method="post" action="{{ route('deletar.rmas', [$rmas->id]) }}">
                      @csrf
